/**
 * Just enough DOM for a panel test.
 *
 * Hand-built in the same spirit as the fake IndexedDB and deploy-DOM helpers:
 * it models the element surface the UI modules actually touch — element
 * creation, classes, attributes, a delegated click listener and the handful of
 * selectors they query — so rendering can be driven offline. Markup drift in
 * index.html is caught separately, by the tests that read the template itself.
 */
...
/**
 * Build a document out of a flat element spec and install it as the globals a
 * UI module reads.
 *
 * @param {Record<string, { tag?: string, id?: string|null, attrs?: Record<string, string>,
 *                          classes?: string[] }>} spec
 * @returns {{ document: any, nodes: Record<string, any>, root: any, restore: () => void }}
 */
export function installFakeDom(spec) {
    const previous = {
    ...
    for (const [key, { tag = 'div', id = null, attrs = {}, classes = [] }] of Object.entries(spec)) {
    ...
    const document = {
        ...
        /** Fire a document-level event, the way a keypress reaches a dialog. */
        press(key, extra = {}) {
