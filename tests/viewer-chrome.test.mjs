 ...
 * every millimetre of it has to be something a visitor can act on. What it says
 * about the signature is now a mark rather than a sentence, and the room that
 * used to hold a transport label ("From Cache" / "Fresh Download" — a fact
 * about the load, not about the site) holds the two decisions instead.
 *
 * Both of those decisions are claims about what strangers can pull from this
 * browser, so the state of the buttons is asked of the store rather than
 * assumed: these tests pin that a Reseed is only offered when there is
 * genuinely a payload to reseed, and that it says why when there is not.
 */
 ...
 */
async function context({ seeding = false, record = null, payload = null } = {}) {
...
test('a verified publisher is a tick, and the sentence it replaces stays readable', async () => {
    const peerweb = await context();

    peerweb.updateSiteSignatureBadge({ verified: true, label: 'Verified publisher: 0xfeed0000…' });

    const badge = dom.nodes.badge;
    // The whole chip is the mark. No address, no wording — it was spending the
    // width a visitor needs for the buttons next to it.
    assert.equal(badge.textContent, '✔');
    assert.equal(badge.className, 'viewer-verified is-verified');
    // A mark alone explains nothing, so what it means is still on the element
    // for a pointer and for a screen reader.
    assert.equal(badge.getAttribute('title'), 'Verified publisher: 0xfeed0000…');
    assert.equal(badge.getAttribute('aria-label'), 'Verified publisher: 0xfeed0000…');
    assert.equal(badge.getAttribute('role'), 'img');
});

test('unverified is never silent', async () => {
    const peerweb = await context();

    peerweb.updateSiteSignatureBadge({ verified: false, label: 'Publisher: unverified' });
    ...
    assert.equal(dom.nodes.badge.getAttribute('aria-label'), 'Publisher: unverified');
});
...
test('Reseed is offered when this browser actually holds the payload', async () => {
    ...
    assert.match(dom.nodes.reseed.textContent, /Reseed/);
...
test('a site stored here but off the air is a resume, not a second copy', async () => {
    ...
    assert.equal(dom.nodes.reseed.disabled, false);
    assert.match(dom.nodes.reseed.textContent, /Resume seeding/);
});
