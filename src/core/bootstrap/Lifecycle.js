...
export async function createGoFileMirror(hash) {
    ...
    if (!this.lastPublishCandidate?.payloadFiles) {
        throw new Error('GoFile mirroring is unavailable after restoring this deployment; WebTorrent remains available.');
    }
...
export async function restoreDeploySession() {
    ...
    try {
        ...
        const mirrorToggle = /** @type {HTMLInputElement | null} */ (
            document.getElementById('deploy-gofile-mirror')
        );
        if (mirrorToggle) {
